...
### 1. Advance the patrol through the wisp reconcile guard

```bash
if [ -n "${GC_BEAD_ID:-}" ]; then
  "$GC_CITY/assets/scripts/witness-wisp-reconcile.sh" --formula mol-refinery-patrol --binding-prefix '{{ .BindingPrefix }}' --var target_branch={{ .DefaultBranch }} --var rig_name={{ .RigName }} --current-wisp "$GC_BEAD_ID"
else
  "$GC_CITY/assets/scripts/witness-wisp-reconcile.sh" --formula mol-refinery-patrol --binding-prefix '{{ .BindingPrefix }}' --var target_branch={{ .DefaultBranch }} --var rig_name={{ .RigName }}
fi
RECONCILE_STATUS=$?
if [ "$RECONCILE_STATUS" -ne 0 ]; then
  echo "Wisp reconcile failed; current wisp was not retired."
  exit 1
fi
```

The reconcile guard resolves the current wisp without a server-side status
filter, adopts an already-poured successor when one exists, and pours only
when no successor is live. It assigns and reads back the successor before
retiring the current wisp, rolling back on a failed assignment. Do not replace
it with an open-coded lookup, pour, or burn. A non-zero result is a hard stop;
do not burn anything manually.
...
- Your session is approaching its context limit (handle that via Rule 2,
  not by skipping the reconcile call).
...
message and stopping without advancing through reconcile. There is no "session
done" state for a refinery patrol — only "next wisp ready" or "wedged".
...
shortcuts or summarizing prematurely. If context feels heavy, then **advance
through the reconcile guard, THEN request restart**:
...
RECONCILE_STATUS=$?
if [ "$RECONCILE_STATUS" -ne 0 ]; then
  echo "Wisp reconcile failed; not requesting restart."
  exit 1
fi
...
after reconcile retires the current wisp. On the normal path, the controller kills and
...
# Step 1: Reconcile the startup wisp before considering a pour. The helper
# returns a structured result: only `status=absent` maps to an empty WISP.
STARTUP_RESULT=$("$GC_CITY/assets/scripts/witness-wisp-reconcile.sh" --startup --formula mol-refinery-patrol --binding-prefix '{{ .BindingPrefix }}' --var target_branch={{ .DefaultBranch }} --var rig_name={{ .RigName }})
RECONCILE_STATUS=$?
if [ "$RECONCILE_STATUS" -ne 0 ]; then
  echo "Startup wisp reconciliation failed; refusing to pour."
  exit 1
fi
WISP=$(printf '%s' "$STARTUP_RESULT" | jq -r '
  if .status == "ready" and (.id | type) == "string" and (.id | length) > 0 then .id
  elif .status == "absent" and .id == null then empty
  else error("unexpected startup reconciliation result")
  end')
PARSE_STATUS=$?
if [ "$PARSE_STATUS" -ne 0 ]; then
  echo "Startup wisp reconciliation returned an invalid result; refusing to pour."
  exit 1
fi
if [ -z "$WISP" ]; then
  ...
  if [ -z "$WISP" ]; then
    echo "Could not pour initial refinery wisp."
    exit 1
  fi
...
|------------|----------------|
| Advance patrol | Follow Patrol Lifecycle Discipline Rule 1: run `witness-wisp-reconcile.sh` and let it validate assignment before retiring the current wisp. Never run a standalone pour or burn. |
